...
interface EmojibaseMessage { key: string; message: string; }
interface EmojibaseEmoji {
  emoji?: string; label?: string; tags?: string[]; group?: number; version?: number; type?: number;
  skins?: Array<{ emoji?: string; tone?: number }>;
}

export async function loadEmojiData(locale = "en", baseUrl = DEFAULT_URL, signal?: AbortSignal): Promise<EmojiData> {
  const root = baseUrl.replace(/\/$/, "");
  const [emojiResponse, messagesResponse] = await Promise.all([
    fetch(`${root}/${locale}/data.json`, { signal }), fetch(`${root}/${locale}/messages.json`, { signal }),
  ]);
  if (!emojiResponse.ok || !messagesResponse.ok) throw new Error(`Unable to load Emojibase data for locale "${locale}".`);

  const rawEmojis = await emojiResponse.json() as EmojibaseEmoji[];
  const messages = await messagesResponse.json() as { groups?: EmojibaseMessage[]; skinTones?: EmojibaseMessage[] };
  ...
  const skinTones = Object.fromEntries((messages.skinTones ?? []).flatMap((tone) => {
    const name = tone.key as SkinTone;
    return name !== "none" && Object.values(toneByNumber).includes(name) ? [[name, tone.message]] : [];
  ...
  const emojis = rawEmojis.flatMap((item) => {
    ...
    const skins = Object.fromEntries((item.skins ?? []).flatMap((skin) => {
    ...
    return [{
      emoji: item.emoji, label: item.label ?? item.emoji, category: item.group, version: item.version ?? 0,
      tags: item.tags ?? [], countryFlag: item.type === 4 ? true as const : undefined,
